...
exports.topicsRouter = (0, context_1.router)({
    /**
     * Get topic details and definition
     */
    getTopicDetails: context_1.protectedProcedure
        ...
        try {
            ...
            const coOccurringTopics = new Map();
            relatedPapers.forEach(paper => {
                const title = paper.title || "";
                const words = title.toLowerCase()
                    .replace(/[^\w\s]/g, "")
                    .split(/\s+/)
                    .filter((word) => word.length > 3 && word !== topicLower);
                words.forEach((word) => {
     ...
     */
    searchTopics: context_1.protectedProcedure
        .input(zod_1.z.object({
     ...
     */
    getPopularTopics: context_1.protectedProcedure
        .input(zod_1.z.object({
     ...
     */
    getTopicSuggestions: context_1.protectedProcedure
        .query(async ({ ctx }) => {
        try {
            const userId = ctx.user.uid;
            const db = (0, firestore_1.getFirestore)();
