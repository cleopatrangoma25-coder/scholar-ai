...
async function performVectorSearch(query, documentMetadata, maxResults) {
    try {
...
async function performMetadataSearch(query, documentMetadata, maxResults) {
    const results = [];
    const searchQuery = query.toLowerCase();
    for (const [docId, docData] of documentMetadata) {
        if (!docData)
