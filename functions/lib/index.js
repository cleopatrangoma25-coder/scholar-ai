...
exports.searchDocumentsAI = exports.realVertexAISearch = exports.vertexAISearch = exports.exploreDatabase = exports.getSystemStatus = exports.ragEngineSearch = exports.enhancedSearch = exports.getDocumentsForAnalysis = exports.enhancedDocumentUpload = exports.getDocuments = exports.uploadDocument = exports.searchDocuments = exports.healthCheck = void 0;
    ...
    try {
        ...
        try {
            ...
            for (const file of files) {
                ...
                if (matchFound) {
                    ...
                    results.push({
                        ...
                        type: metadata.contentType || ((_a = filename.split('.').pop()) === null || _a === void 0 ? void 0 : _a.toUpperCase()) || 'Unknown',
            ...
            if (isCorpusOverviewQuery) {
                ...
                if (javaDocs > 0)
                    aiResponse += `• **Java Programming:** ${javaDocs} documents (lectures, labs, tests, fundamentals)\n`;
                if (networkingDocs > 0)
                    aiResponse += `• **Computer Networking:** ${networkingDocs} documents (ICT242 lectures, protocols, security)\n`;
                if (operatingSystemDocs > 0)
                    aiResponse += `• **Operating Systems:** ${operatingSystemDocs} documents (ICT222 lectures, exams, practice questions)\n`;
                if (databaseDocs > 0)
                    aiResponse += `• **Database Design:** ${databaseDocs} documents (ICT271 lectures, design principles)\n`;
                if (economicsDocs > 0)
                    aiResponse += `• **Economics:** ${economicsDocs} documents (textbooks, research papers)\n`;
                if (lawDocs > 0)
                    aiResponse += `• **Law & Legal Studies:** ${lawDocs} documents (case studies, legal principles)\n`;
                if (mlDocs > 0)
                    aiResponse += `• **Machine Learning:** ${mlDocs} documents (algorithms, research papers)\n`;
                if (otherDocs > 0)
                    aiResponse += `• **Other Academic Topics:** ${otherDocs} documents (various subjects)\n`;
            ...
            console.error('Error details:', {
                ...
                code: (storageError === null || storageError === void 0 ? void 0 : storageError.code) || 'No code',
                details: (storageError === null || storageError === void 0 ? void 0 : storageError.details) || 'No details'
            });
...
exports.enhancedDocumentUpload = functions.https.onRequest(async (req, res) => {
    // Gen 2 functions handle CORS automatically
    try {
        ...
        await file.save(content, {
            metadata: {
                contentType: `application/${fileType}`,
                metadata: Object.assign({ documentId: documentId, uploadDate: documentData.uploadDate }, metadata)
            }
        });
...
exports.getDocumentsForAnalysis = functions.https.onRequest(async (req, res) => {
    // Gen 2 functions handle CORS automatically
    try {
        const db = admin.firestore();
...
exports.enhancedSearch = functions.https.onRequest(async (req, res) => {
    // Gen 2 functions handle CORS automatically
    try {
...
exports.ragEngineSearch = functions.https.onRequest(async (req, res) => {
    // Gen 2 functions handle CORS automatically
    try {
...
exports.exploreDatabase = functions.https.onRequest(async (req, res) => {
    // Gen 2 functions handle CORS automatically
    try {
...
exports.vertexAISearch = functions.https.onRequest(async (req, res) => {
    // Gen 2 functions handle CORS automatically
    try {
        const { query, searchType = 'vector' } = req.body;
        if (!query) {
        ...
        for (const doc of documentsSnapshot.docs) {
            ...
            if (matchFound) {
                ...
                let actualContentChunks = [];
                // If document has chunks, use them
                if (docData.chunks && Array.isArray(docData.chunks) && docData.chunks.length > 0) {
                    actualContentChunks = docData.chunks.map((chunk) => chunk.text || chunk.content || chunk.toString());
                }
...
var real_vertex_ai_search_1 = require("./real-vertex-ai-search");
Object.defineProperty(exports, "realVertexAISearch", { enumerable: true, get: function () { return real_vertex_ai_search_1.realVertexAISearch; } });
// NEW: AI-Powered Document Search with Intelligent Responses (Like Gemini 2.5 Flash)
exports.searchDocumentsAI = functions.https.onRequest(async (req, res) => {
    var _a;
    // Handle CORS preflight
    if (req.method === 'OPTIONS') {
        res.set('Access-Control-Allow-Origin', 'https://scholar-ai-1-prod.web.app');
