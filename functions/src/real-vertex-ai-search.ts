...
export const realVertexAISearch = functions.https.onRequest(async (req, res) => {
  setCorsHeaders(res);
...
async function performRealVectorSearch(query: string, documentsSnapshot: any, maxResults: number) {
  try {
...
function simulateVectorSearchResults(queryEmbedding: number[], maxResults: number) {
  const results = [];
...
async function enrichResultsWithContent(searchResults: any[], documentsSnapshot: any) {
  try {
    ...
    for (const result of searchResults) {
      try {
        ...
        const documentId = result.restrictMetadata?.document_id || 
                          result.datapoint?.datapointId?.replace('simulated-result-', 'doc-') ||
                          `doc-${Math.floor(Math.random() * documentsSnapshot.size)}`;
        
        const docData = documentMap.get(documentId) || 
                       documentsSnapshot.docs[Math.floor(Math.random() * documentsSnapshot.size)]?.data();
        
        if (docData) {
          // Calculate similarity score
          const similarityScore = parseFloat(result.restrictMetadata?.similarity_score) || 
                                (1 - (result.distance || 0));
...
async function getActualContentChunks(documentId: string, docData: any) {
  try {
