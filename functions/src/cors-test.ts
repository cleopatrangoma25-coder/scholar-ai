...
export const corsTest = onRequest({
  memory: "256MiB",
  timeoutSeconds: 60,
  cors: true
}, async (req, res) => {
  ...
  res.json({
    status: "success",
    message: "CORS Test API with real document search is running",
    time: new Date().toISOString(),
    bucket: documentsBucket,
...
async function searchDocumentsInBucket(files: any[], query: string): Promise<Array<{
  paperId: string;
  title: string;
  authors: string[];
  pageNumber: number;
...
function calculateRelevanceScore(chunk: string, query: string): number {
...
function generateAnswerFromDocuments(query: string, sources: Array<{
  paperId: string;
  title: string;
  authors: string[];
  pageNumber: number;
  content: string;
  score: number;
}>, scope: string, totalDocuments: number): string {
  if (sources.length === 0) {
...
Try rephrasing your question or upload documents that contain information about "${query}".`;
  ...
  const sourceTexts = sources.map(source => 
    `From "${source.title}" (Page ${source.pageNumber}):\n${source.content}`
  ).join('\n\n');
